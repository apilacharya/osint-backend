...
export const formatFindingForFeed = (finding) => {
...
export const fetchDiscoveryFeed = async () => {
    const settled = await Promise.allSettled(discoveryQueries.map((item) => item.adapter.search({ query: item.query, context: undefined })));
