import type { AdapterFinding, ResolvedFinding, SearchContext } from "../aggregation/types.js";

const normalize = (value: string): string => value.trim().toLowerCase();

const textIncludes = (value: string, tokens: string[]): number =>
  tokens.reduce((acc, token) => (value.includes(token) ? acc + 1 : acc), 0);

export const resolveEntityFindings = (
  query: string,
